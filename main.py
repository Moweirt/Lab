...
st.markdown("Для лучшего понимания темы предлагаем Вам найти коэффициент сопротивления **_b_** и сравнить его с табличным.")
...
b_1 = st.number_input("Введите коэффициент пропорциональности b между силой трения и скоростью движения груза c клавиатуры")
...
if (b_1 < 0.030 and b_1 > 0.010):
    resp2 += 6
    st.success(f"Всё правильно! Вы верно определили коэффициент! + ( {resp2} балл(а) )")

# конец

if (resp+resp2 >= 8) :
    st.success(f"Отлично!! Вы справились с лабораторной работой на 100%! У вас ( {resp+resp2} ) баллов. ")
    st.baloons

if (resp+resp2 < 7 and resp+resp2 >= 5) :
    st.warning(f"Неплохо! Средний результат. У вас ( {resp+resp2} ) баллов.")

if (resp+resp2 < 5) :
    st.error(f"Неудовлетворительно! Вы не справились с работой. У вас ( {resp+resp2} ) баллов.")



#1) проверить теорию(+ добавить формулы и указания)
#2) добавить табл. с данными
#3) Оформить конец упр.2
#4) Закончить лабу(вывести баллы и шарики)
#!!!!! Добавить Милоса