...
st.markdown(
    "Здесь **_A_** — амплитуда колебаний; **_ω0_** — круговая частота; **_t_** — время; **_φ0_** — начальная фаза колебаний.")
...
st.markdown(
    "Пружинный маятник состоит из тела массы **_m_** и легкой пружины с коэффициентом жесткости **_k_**. В общем случае "
...
st.markdown(
    "где **_b_** – коэффициент сопротивления, **_V_** – скорость системы, действует ещё сила упругости. Тогда уравнение "
...
st.markdown(
    "циклическая частота собственных колебаний маятника, получим дифференциальное уравнение затухающих колебаний")
...
st.latex("ω = \sqrt{k/m - b^2/4m^2}, (5)")

st.latex("T = 2π/\sqrt{k/m - b^2/4m^2}, (6)")

st.markdown(
    "Время, за которое амплитуда затухающих колебаний уменьшается в **_e_** раз, называется временем релаксации **_τ_**. "
    "Время релаксации **_τ_** связано с коэффициентом затухания **_β_** соотношением")
...
st.markdown(
    "Логарифм отношения амплитуды **_A1(t)_** затухающих колебаний в момент времени **_t_** к амплитуде **_A2(t+T)_** "
    "колебаний через время, равное периоду **_T_** называется логарифмическим декрементом затухания.")

st.latex("δ = ln A_1(t)/A_2(t+T) = ln A_0e^{-βt}/A_0e^{-β(t+T)} = βT = b/2m * 2π/\sqrt{k/m - b^2/4m^2}, (8) ")

st.markdown(
    "При очень больших коэффициентах сопротивления **_b_** (когда **_k/m << b^2/4m^2_**), несмотря на наличие сил, "
...
st.markdown(
    "Определив на опыте две последующие амплитуды **_An_** и **_An+1_** и зная массу колеблющейся системы **_m_** и период "
...
st.markdown("т.е. логарифмический декремент затухания есть величина, обратная числу колебаний, совершаемых системой за "
            "время, в течение которого амплитуда колебаний убывает в **_e_** раз.")

st.markdown(
    "Затухание колебаний по экспоненциальному закону происходит в том случае, когда сила трения пропорциональна "
...
st.markdown("**_Примеры затухающих колебаний в жизни:_**")

st.markdown(
    "Колебания, производимые детьми, сидящими на весенних лошадях в парке, — это то, что мы наблюдаем регулярно. "
...
st.markdown(
    " **_ЗАДАНИЕ_**: Меняя один из параметров системы и изучая изменения графика, определите, как они влияют на её харатеристики")
...
ch_mass = st.multiselect("Влияние массы на систему",
                         ["Увеличивает период", "Уменьшает период", "Не влияет на период", "Увеличивает амплитуду",
                          "Уменьшает амплитуду", "Не влияет на амплитуду"])
ch_rad = st.multiselect("Влияние радиуса шарика на систему",
                        ["Увеличивает период", "Уменьшает период", "Не влияет на период", "Увеличивает амплитуду",
                         "Уменьшает амплитуду", "Не влияет на амплитуду"])
ch_rig = st.multiselect("Влияние жесткости пружины на систему",
                        ["Увеличивает период", "Уменьшает период", "Не влияет на период", "Увеличивает амплитуду",
                         "Уменьшает амплитуду", "Не влияет на амплитуду"])
ch_vis = st.multiselect("Влияние динамической вязкости жидкости на систему",
                        ["Увеличивает период", "Уменьшает период", "Не влияет на период", "Увеличивает амплитуду",
                         "Уменьшает амплитуду", "Не влияет на амплитуду"])
...
for i in ch_mass:
    if (i == "Увеличивает период" or i == "Не влияет на амплитуду"): resp1 += 1
for i in ch_rad:
    if (i == "Не влияет на период" or i == "Уменьшает амплитуду"): resp1 += 1
for i in ch_rig:
    if (i == "Уменьшает период" or i == "Не влияет на амплитуду"): resp1 += 1
for i in ch_vis:
    if (i == "Не влияет на период" or i == "Уменьшает амплитуду"): resp1 += 1
if (resp1 > 0 and resp1 <= 3):
    st.error(f"Есть ошибки (+ {resp1 / 2} балл(а))")
elif (resp1 >= 4 and resp1 < 7):
    st.warning(f"Вы неплохо справились (+ {resp1 / 2} балла)")
elif (resp1 > 7 and resp1 < 8):
    st.success(f"Почти всё верно! Отлично!(+ {resp1 / 2} балла)")
elif (resp1 == 8):
    st.success(f"Абсолютно верно! Вы молодец!(+ {resp1 / 2} балла)")

resp1 = resp1 / 2

# Упражение 2

st.subheader("Упражнение №2")
st.markdown(
    " **_ЗАДАНИЕ_**: Ваш коллега Андрей провёл измерения на Физфаке престижного Мордовского Государственного Университета, но не обладает "
    "достаточными знаниями, чтобы самому выполнить необходимые вычисления. Он просит вашей помощи. Известно, что измерения проводились "
    "с шариком радиуса 10мм , его масса 140г, при этом ранее Андрей установил, что коэффициент жесткости пружины 12Н/м")

st.image("table.png")
# теория для расчётов

st.info("В процессе подсчётов вам понадобятся некоторые формулы. В том случае, если вы раньше не делали лабораторных работ"
    "вы можете посмотреть их ниже")

st.latex("δ = 5ln2/\sum_{i=1}^{5}n_i")

st.latex("T_a = \sum_{i=1}^{5}t_n/\sum_{i=1}^{5}n_i")

st.latex("b_{1(2)} = 2(m_3+m_0)δ_{1(2)}/T_{a1(2)}")

st.header("Анализ коэффициентов **_b1_** и **_b2_**")

st.latex("δb^* = 2(b_1 - b_2)/(b_1 + b_2)")

<<<<<<< HEAD
st.markdown("Если **_δb*_** более 0,1, то можно сделать вывод о несовпадении коэффициентов **_b1_** и **_b2_**.")
=======
st.markdown("Если **_δb*_** более 0,1, то можно сделать вывод о несовпадении коэффициентов **_b_1_** и **_b_2_**.")
>>>>>>> f848c82f8b5d01c505b2ce1cfcdebbda94875d22
...
b_1 = st.number_input("Введите результат ваших подсчётов коэффициента b_1 (г/с)")

if (b_1 < 22 and b_1 > 18):
    resp2_1 += 3
    st.success(f"Всё правильно! Вы верно определили коэффициент!  (+ {resp2_1 / 2} балла )")
elif(b_1 > 0):
    st.error("Коэффициент вычислен неверно!")

b_2 = st.number_input("Введите результат ваших подсчётов коэффициента b_2 (г/с)")

if (b_2 < 20 and b_2 > 16):
    resp2_2 += 3
    st.success(f"Всё правильно! Вы верно определили коэффициент!  (+ {resp2_2 / 2} балла )")
elif (b_1 > 0):
    st.error("Коэффициент вычислен неверно!")
...
st.markdown("**_ДОП.ЗАДАНИЕ_**: Используя интернет-ресурсы найдите формулу Стокса, она поможет определить, какая жидкость "
            "была в лабораторной работе Андрея (Учтите, что температура жидкости может быть разной)")

st.image("table2.png")

liq = st.text_input("Введите название жидкости")

if (liq == "Глицерин" or liq == "глицерин"):
    resp2_3 += 6
    st.success(f"Верно! ( + {resp2_3/2} балла)")
elif(liq != ""):
    st.error("Неверно! Попробуйте ещё раз (+ 0 баллов)")

st.markdown(
    "С учётом результата ваших вычислений график амплитуды колебаний шарика в лабораторной работе Андрея выглядит так:")
...
resp2 = (resp2_1 + resp2_2 + resp2_3) / 2

if (resp2 > 0 and resp2 <= 2):
    st.error(f"Вы старались, но где-то есть ошибки. (Упраженние №2: {resp2} балл(а) )")
if (resp2 > 2 and resp2 <= 4):
    st.warning(f"Почти всё правильно! Вы хорошо справились (Упражнение №2: {resp2} балла )")
if (resp2 > 4 and resp2 <= 6):
    st.success(f"Всё правильно! Вы отлично справились! (Упражнение №2: {resp2} балла(ов) )")
...
if (resp >= 8):
    st.success(f"Отлично!! Вы сделали лабораторную работу верно. У вас  {resp}/10 баллов. ")
    if (resp == 10):
        st.balloons()
        st.warning("Если вы любите рисковать попробуйте ввести ниже FLEX")

if (resp < 8 and resp >= 5):
    st.warning(
        f"Здорово! Вы усвоили большинство знаний. Вы справились с лабораторной работой на {resp * 10}%! У вас  {resp}/10 баллов.")

if (resp > 0 and resp < 5):
    st.error(f"Видимо, вам что-то непонятно! Попробуйте изучить материал внимательней. У вас  {resp}/10 баллов.")

cheet_code = st.text_input("Введите authors")
if cheet_code == "authors":
    st.info("Над проектом работали Симонов Алексей, Фотиадис София и Тенихин Ярослав")
if cheet_code == "FLEX":
    st.image("ricflex.gif")
    st.audio("Flex.mp3")