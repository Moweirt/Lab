...
st.info(
    "В процессе подсчётов вам понадобятся некоторые формулы. В том случае, если вы раньше не делали лабораторных работ"
...
b_1 = st.number_input("Введите результат ваших подсчётов коэффициента b_1")

if (b_1 < 0.022 and b_1 > 0.018):
    resp2_1 += 3
    st.success(f"Всё правильно! Вы верно определили коэффициент!  (+ {resp2_1 / 2} балла )")
elif(b_1 > 0):
    st.error("Коэффициент вычислен неверно!")

b_2 = st.number_input("Введите результат ваших подсчётов коэффициента b_2")

if (b_2 < 0.020 and b_2 > 0.016):
    resp2_2 += 3
