...
st.markdown("где **_b_** – коэффициент сопротивления, **_V_** – скорость системы, действует ещё сила упругости. Тогда уравнение "
...
st.markdown("При очень больших коэффициентах сопротивления **_b_** (когда **_k/m << b^2/4m^2_**), несмотря на наличие сил, "
...
st.markdown(" **_ЗАДАНИЕ_**: Ваш коллега Андрей провёл измерения на Физфаке престижного Мордовского Государственного Университета, но не обладает"
            ...
            "с шариком радиуса **_10мм_** , его **_масса 140г_**, при этом ранее Андрей установил, что коэффициент жесткости пружины **_12Н/м_**")
...
st.latex("T_a = \sum_{i=1}^{5}t_n_i/\sum_{i=1}^{5}n_i")
...
if (b_1 < 0.022 and b_1 > 0.018):
    resp2_1 += 3
    st.success(f"Всё правильно! Вы верно определили коэффициент! + ( {resp2_1/2} балл(а) )")

b_2 = st.number_input("Введите результат ваших подсчётов коэффициента b_2")

if (b_2 < 0.020 and b_2 > 0.016):
    resp2_2 += 3
    st.success(f"Всё правильно! Вы верно определили коэффициент! + ( {resp2_2/2} балл(а) )")
...
st.markdown("С учётом результата ваших вычислений график амплитуды колебаний шарика в лабораторной работе Андрей выглядит так:")
...
if (resp >= 8) :
    st.success(f"Отлично!! Вы сделали лабораторную работу абсолютно верно. У вас  {resp}/10 баллов. ")
    if(resp == 10): st.ballons

if (resp < 8 and resp >= 5) :
    st.warning(f"Здорово! Вы усвоили большинство знаний. Вы справились с лабораторной работой на {resp*10}%! У вас  {resp}/10 баллов.")
